...
import { minusCart, plusCart, removeCart } from '../../redux/slices/cartSlice'

function CartItem({ id, title, sum, count, imageUrl, type, size })
{
    const dispatch = useDispatch()
    const onRemove = () =>
    {
        if (window.confirm('Вы действительно хотите удалить товар?')) dispatch(removeCart(id))
    }
    return (
        <div className="cart__item">
            ...
            <div className="cart__item-count">
                <div onClick={() => dispatch(count > 1 ? minusCart(id) : removeCart(id))} className="button button--outline button--circle cart__item-count-minus">
                ...
                <b>{count}</b>
                <div onClick={() => dispatch(plusCart(id))} className="button button--outline button--circle cart__item-count-plus">
